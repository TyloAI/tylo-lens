...
export function createFetchInterceptor(fetchImpl, options) {
    const { trace, parentId, getParentId, onSpanStart, onSpanUpdate, onSpanEnd, captureBody = false, redactHeaders = true, shouldTrace, captureSSE = false, sse, } = options;
    return (async (input, init) => {
        ...
        try {
            ...
            void (async () => {
                ...
                const flushLine = (line, dataLines) => {
                    if (line === '') {
                ...
                try {
                    while (true) {
                        ...
                        while ((idx = buffered.indexOf('\n')) !== -1) {
                            ...
                            const res = flushLine(line, dataLines);
                    ...
                    span.durationMs = durationMs(start, end);
                    if (truncated)
                        span.meta = { ...(span.meta ?? {}), sseTruncated: true };
                    onSpanEnd?.(span);
                    try {
                        reader.releaseLock();
                    }
