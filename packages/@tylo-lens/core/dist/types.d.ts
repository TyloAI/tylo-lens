...
export type LensPiiFindingType = 'email' | 'phone' | 'credit_card' | 'ssn' | 'ip_address' | 'api_key';
...
export type LensPricingTable = Record<string, {
    currency?: 'USD' | (string & {});
    pricePer1KInput: number;
    pricePer1KOutput: number;
}>;
...
export type LensEventHandler<T extends LensEventType> = (event: Extract<LensEvent, {
    type: T;
}>) => void;
