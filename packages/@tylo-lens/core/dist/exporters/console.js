...
export function consoleExporter(options) {
    ...
    return {
        ...
        export: (trace) => {
            ...
            console.log(`[tylo-lens] trace=${trace.traceId} spans=${s.spans} tokens=${s.totalTokens} cost=${s.totalCost.toFixed(6)} pii=${s.hasPII ? 'yes' : 'no'}`);
            if (verbose) {
                // eslint-disable-next-line no-console
