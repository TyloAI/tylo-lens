import type { LensSpan, LensTrace, ProtoethikTransparency, ProtoethikWeights } from '../types.js';
import { estimateTokens } from '../collectors/token-estimator.js';
import { scanForPII } from './pii.js';

const DEFAULT_WEIGHTS: ProtoethikWeights = {
  clarity: 1,
  pii: 1,
};

export function computeClarityScore(text: string): number {
  const t = (text ?? '').trim();
  if (!t) return 0;
...
function getSpanTokens(span: LensSpan): number {
  if (span.usage?.totalTokens != null) return safeNumber(span.usage.totalTokens);
...
function getSpanOutputTokens(span: LensSpan): number {
  if (span.usage?.outputTokens != null) return safeNumber(span.usage.outputTokens);
  const output = span.output?.text ?? '';
  return estimateTokens(output);
}

function getSpanPiiCount(span: LensSpan): number {
...
export function computeProtoethikTransparency(
  trace: LensTrace,
  options?: { weights?: Partial<ProtoethikWeights> },
): ProtoethikTransparency {
  const weights: ProtoethikWeights = { ...DEFAULT_WEIGHTS, ...(options?.weights ?? {}) };
  ...
  return {
    ...
    weights,
    formula:
      'T_score = (Σ(C_clarity · W_c) − Σ(P_pii · W_p)) / T_tokens, where C_clarity = clarity(output) · output_tokens and P_pii = pii_count · 1000',
  };
}

export function annotateProtoethikAnalysis(
  trace: LensTrace,
  options?: { weights?: Partial<ProtoethikWeights> },
): LensTrace {
  const weights: ProtoethikWeights = { ...DEFAULT_WEIGHTS, ...(options?.weights ?? {}) };
