...
export function scanForPII(text: string): LensPiiFinding[] {
  if (!text) return [];
  const findings: Record<LensPiiFindingType, number> = {
    email: 0,
    phone: 0,
    credit_card: 0,
    ssn: 0,
  ...
  for (const rule of RULES) {
    ...
    if (matches?.length) findings[rule.type] += matches.length;
  }

  return (Object.entries(findings) as Array<[LensPiiFindingType, number]>)
...
function maskPreview(input: string, maxVisible = 2): string {
  const s = input ?? '';
  if (s.length <= maxVisible * 2) return '*'.repeat(Math.min(12, s.length));
...
function redactedMarker(type: LensPiiFindingType, match: string, mode: LensRedactionMode): string {
  if (mode === 'hash') return `[REDACTED:${type}:${hashString(match)}]`;
...
export function collectPIIEvidence(
  text: string,
  field: LensPiiEvidenceField,
  options?: { mode?: LensRedactionMode; includeRawMatch?: boolean; contextChars?: number; maxItems?: number },
): LensPiiEvidence[] {
  if (!text) return [];
...
export function redactPII(text: string, mode: LensRedactionMode = 'mask'): string {
  if (!text) return text;
  if (mode === 'none') return text;

  const replacer = (type: LensPiiFindingType, match: string): string => {
    if (mode === 'hash') {
...
export function riskFromFindings(findings: LensPiiFinding[]): 'low' | 'medium' | 'high' {
  const total = findings.reduce((sum, f) => sum + f.count, 0);
  if (total === 0) return 'low';
  // Credit card / SSN / API keys are treated as high risk.
  const hasHigh =
    findings.some((f) => f.type === 'credit_card' || f.type === 'ssn' || f.type === 'api_key') ||
    total >= 5;
